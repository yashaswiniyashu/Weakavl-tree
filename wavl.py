...
class WAVLNode(bst.Node):
    def __init__(self, key, name,address,mob_num,course,m1,m2,m3,total,percent, left=None, right=None, rank=0, parent=None):
...
class WAVL(bst.BST):
    ...
    def insert(self, key,name,mob_num,course,address,m1,m2,m3,total,percent,):
        if self.root:
            self._insert(key,name,address,mob_num,course,m1,m2,m3,total,percent, self.root)
        else:
            self.root = WAVLNode(key,name,address,mob_num,course,m1,m2,m3,total,percent)

    def _insert(self, key,name,address,mob_num,course,m1,m2,m3,total,percent,node):
        if key < node.key:
            if node.left:
                self._insert(key,name,address,mob_num,course,m1,m2,m3,total,percent, node.left)
            else:
                node.left = WAVLNode(key,name,address,mob_num,course,m1,m2,m3,total,percent, parent=node)
                self.insert_rebalance(node.left)
        else:
            if node.right:
                self._insert(key,name,address,mob_num,course,m1,m2,m3,total,percent, node.right)
            else:
                node.right = WAVLNode(key,name,address,mob_num,course,m1,m2,m3,total,percent, parent=node)
    ...
    def _search(self, key, node):
        if not node:
            ...
            print("ROLL NUMBER:-",node.key)
            print("NAME :",node.name)
            print("ADDRESS :",node.address)
            print("COURSE :",node.mob_num)
            print("CONTACT NUMBER :",node.course)
            print("MARKS IN THREE SUBJECTS: ")
            print("FIRST SUBJECT:",node.m1)
            print("SECOND SUBJECT:",node.m2)
            print("THIRD SUBJECT:",node.m3)
            print("TOTAL MARKS: ",node.total)
            print("PERCENTAGE:  ",node.percent)
